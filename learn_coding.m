function Sout = learn_coding(B, X, alpha, mu, gamma, L, M, Sinit)
...
%     if mod(i, 100)==0, fprintf('.'); end %fprintf(1, 'l1ls_featuresign: %d/%d\r', i, size(X,2)); end
    if use_Sinit
        ...
        Sout(:,i) = ls_featuresign_sub (B, S, X(:,i), BtB, BtX(:,i), L, M, i, alpha, mu, gamma, sinit);
    else
        Sout(:,i) = ls_featuresign_sub (B, Sout, X(:,i), BtB, BtX(:,i), L, M, i, alpha, mu, gamma);
    end
end
...
function [s, fobj] = ls_featuresign_sub (B, S, x, BtB, Btx, L, M, i, alpha, mu, gamma, sinit)
...
for iter=1:ITERMAX
    ...
    grad = BtB*sparse(s)+(alpha*L(i,i)+mu*M(i,i))*sparse(s)+alpha*P+mu*Q - Btx;
    theta = sign(s);
    ...
    while 1
        ...
        [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, P, L, Q, M, i, theta, act, act_indx1, alpha, mu, gamma);
...
function [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, P, L, Q, M, idx, theta, act, act_indx1, alpha, mu, gamma)
...
a = (alpha*L(idx,idx)+mu*M(idx,idx))*ones(size(act_indx1));
%a = L(idx,idx)*alpha*eye(size(act_indx1,1));
% call matlab optimization solver..
s_new = (BtB2+diag(a)) \ ( Btx(act_indx1) - gamma.*theta2 - alpha*P(act_indx1) - mu*Q(act_indx1)); % RR
...
a= 0.5*sum((B(:,act_indx1)*(s_new-s2)).^2)+0.5*(alpha*L(idx,idx)+mu*M(idx,idx))*(s_new-s2)'*(s_new-s2);
b= (s_new-s2)'*(alpha*P(act_indx1)+mu*Q(act_indx1)-Btx(act_indx1)) + (s2'*BtB2+ (alpha*L(idx,idx)+mu*M(idx,idx))*s2')*(s_new-s2);
...
function [f, g] = fobj_featuresign(s, B, x, BtB, Btx, P, L, Q, M, i, alpha, mu, gamma)

f= 0.5*(norm(x-B*s)^2+alpha*L(i,i).*(s'*(s+2*P))+mu*M(i,i).*(s'*(s+2*Q)));
f= f+ gamma*norm(s,1);

if nargout >1
    g = BtB*s+alpha*L(i,i)*s+alpha*P+mu*M(i,i)*s+mu*Q - Btx;
    g= g+ gamma*sign(s);
end
